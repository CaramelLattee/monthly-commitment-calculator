...
MONO = "Consolas"  # monospace — numbers, codes, dates
UI   = "Segoe UI"  # ui labels
# ──────────────────────────────────────────────────────────────────────────────
...
def _panel(parent, **kw):
    """Styled card frame with consistent border and background."""
    defaults = dict(fg_color=PANEL, border_color=BORDER, border_width=1, corner_radius=6)
    defaults.update(kw)
...
class CalendarPopup(ctk.CTkToplevel):
    ...
    def _build(self):
        ...
        nav = ctk.CTkFrame(self, fg_color=STRIP, corner_radius=6,
                            border_color=BLUE, border_width=1)
        nav.grid(row=0, column=0, padx=12, pady=(12, 6), sticky="ew")
        nav.grid_columnconfigure(1, weight=1)

        ctk.CTkButton(nav, text="◂", width=32, height=28, fg_color=PANEL,
                      hover_color=BLUE_D, text_color=BLUE, border_width=1,
        ...
        for col, d in enumerate(["Mo","Tu","We","Th","Fr","Sa","Su"]):
            ctk.CTkLabel(days_frame, text=d, width=38,
                         ...
                         text_color=MUTED).grid(row=0, column=col, padx=1)
        ...
        ctk.CTkButton(action, text="CLEAR", height=32, fg_color="transparent",
                      ...
                      hover_color=STRIP, corner_radius=4,
                      font=ctk.CTkFont(family=MONO, size=11),
                      command=self._clear).grid(row=0, column=0, padx=(0,4), sticky="ew")
        ctk.CTkButton(action, text="CONFIRM", height=32, fg_color=BLUE,
                      hover_color=BLUE_D, text_color="white", corner_radius=4,
        ...
        for r, week in enumerate(weeks):
            for c, day in enumerate(week):
                ...
                ctk.CTkButton(
                    ...
                    fg_color=fg, hover_color=BLUE_D, corner_radius=4, text_color=txt,
                    font=ctk.CTkFont(family=MONO, size=11),
...
class App(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        # ── HEADER ────────────────────────────────────────────────────────────
        hdr = ctk.CTkFrame(self, fg_color=STRIP, border_color=BLUE,
        ...
        ctk.CTkLabel(hdr, text="◈  MONTHLY COMMITMENT CALCULATOR",
                     font=ctk.CTkFont(family=MONO, size=13, weight="bold"),
                     text_color=BLUE).grid(row=0, column=0, padx=16, pady=10, sticky="w")

        mode_lbl = "☀  LIGHT" if self._dark_mode else "☾  DARK"
        ctk.CTkButton(hdr, text=mode_lbl, width=96, height=28,
                      fg_color=STRIP, hover_color=BLUE_D, border_width=1,
                      border_color=BLUE, text_color=BLUE, corner_radius=4,
                      font=ctk.CTkFont(family=MONO, size=10, weight="bold"),
        ...
        ctk.CTkLabel(hdr, text=date.today().strftime("%a, %d %b %Y").upper(),
                     font=ctk.CTkFont(family=MONO, size=11),
                     text_color=MUTED).grid(row=0, column=2, padx=16, pady=10, sticky="e")
        ...
        self.income_entry = ctk.CTkEntry(
            ...
            font=ctk.CTkFont(family=MONO, size=24, weight="bold"),
            fg_color=STRIP, border_color=BORDER, text_color=TEXT,
            placeholder_text_color=MUTED, height=46, corner_radius=4)
        ...
        ctk.CTkButton(
            ...
            fg_color=STRIP, hover_color="#280010", border_width=1, border_color=PINK,
            text_color=PINK, font=ctk.CTkFont(family=MONO, size=11, weight="bold"),
        ...
        self.lbl_income = self._stat_card(stats, "INCOME",    BLUE,  col=0)
        self.lbl_commit = self._stat_card(stats, "COMMITTED", PINK,  col=1)
        self.lbl_remain = self._stat_card(stats, "REMAINING", GREEN, col=2)
        ...
        ctk.CTkLabel(prog_row, text="LOAD",
                     ...
                     text_color=MUTED, width=38, anchor="w").grid(row=0, column=0, padx=(0, 8))

        self.progress = ctk.CTkProgressBar(prog_row, height=8, corner_radius=4,
                                            fg_color=STRIP, progress_color=BLUE)
        self.progress.grid(row=0, column=1, sticky="ew")
        self.progress.set(0)

        self.lbl_pct = ctk.CTkLabel(prog_row, text="0.0%",
                                     font=ctk.CTkFont(family=MONO, size=10, weight="bold"),
                                     text_color=TEXT, width=48, anchor="e")
        ...
        ctk.CTkButton(exp, text="↓  CSV", width=88, height=28,
                      fg_color=STRIP, hover_color=BLUE_D, border_width=1, border_color=BLUE,
                      text_color=BLUE, corner_radius=4,
                      font=ctk.CTkFont(family=MONO, size=10, weight="bold"),
                      command=self._export_csv).pack(side="left", padx=(0, 6))
        ctk.CTkButton(exp, text="↓  PDF", width=88, height=28,
                      fg_color=PINK, hover_color=PINK_D,
        ...
        ctk.CTkButton(yr, text="◂", width=28, height=26, fg_color=STRIP,
                      hover_color=BLUE_D, text_color=BLUE, border_width=1,
                      border_color=BORDER, corner_radius=4,
                      font=ctk.CTkFont(family=MONO, size=12),
        ...
        entry_kw = dict(fg_color=STRIP, border_color=BORDER, text_color=TEXT,
                        placeholder_text_color=MUTED, corner_radius=4, height=34)
        ...
        self.due_btn = ctk.CTkButton(
            ...
            fg_color=STRIP, hover_color=BLUE_D, border_width=1, border_color=BORDER,
        ...
        ctk.CTkButton(
            ...
            fg_color=BLUE, hover_color=BLUE_D, text_color="white",
        ...
        self.list_header_lbl = ctk.CTkLabel(
            lf, text="COMMITMENTS",
            font=ctk.CTkFont(family=UI, size=10, weight="bold"), text_color=MUTED)
    ...
    def _stat_card(self, parent, label, accent, col):
        frame = ctk.CTkFrame(parent, fg_color=STRIP, border_color=accent,
                              border_width=1, corner_radius=6)
    ...
    def _highlight_month_btn(self):
        for i, btn in enumerate(self.month_btns):
            if i + 1 == self._view_month:
                btn.configure(fg_color=BLUE, hover_color=BLUE_D,
                               text_color="white", border_width=0)
            else:
                btn.configure(fg_color=STRIP, hover_color=BLUE_D,
    ...
    def _on_due_picked(self, d: date | None):
        ...
        if d:
            self.due_btn.configure(text=d.strftime("%d %b %Y"),
                                    text_color=BLUE, border_color=BLUE)
        else:
    ...
    def _reset_all(self):
        ...
        self._new_due_date = None
        self.due_btn.configure(text="  PICK DATE", text_color=MUTED, border_color=BORDER)
    ...
    def _refresh(self):
        ...
        self.lbl_remain.configure(text=fmt(remain),
                                   text_color=GREEN if remain >= 0 else PINK)
        self.progress.set(pct)
        self.lbl_pct.configure(text=f"{pct*100:.1f}%")

        if pct >= 1.0:
            self.progress.configure(progress_color=PINK)
            self.lbl_status.configure(
                text="[!] OVERLOADED — commitments exceed income", text_color=PINK)
        elif pct >= 0.75:
        ...
        # ── Excel-style table ──────────────────────────────────────────────────
        CELL_H  = 20
        HDR_SEP = "#5a9bff"   # vertical separator colour inside header
        fh = ctk.CTkFont(family=MONO, size=11, weight="bold")  # header font
        fm = ctk.CTkFont(family=MONO, size=11)                  # mono data
        fn = ctk.CTkFont(family=UI,   size=12, weight="bold")   # name
        fa = ctk.CTkFont(family=MONO, size=11, weight="bold")   # amount

        # Outer frame — BORDER colour shows through 1-px gaps = grid lines
        tbl = ctk.CTkFrame(self.scroll, fg_color=BORDER, corner_radius=4)
        ...
        def _build_row(parent, bg, num_txt, name_txt, amt_txt, due_txt,
            ...
            r.pack(fill="x", pady=(0, 1))   # 1-px gap = horizontal grid line
            r.pack_propagate(False)          # lock row to exactly CELL_H
            sep_clr = HDR_SEP if is_hdr else BORDER
            fg      = "white"  if is_hdr else TEXT
            fg_m    = "white"  if is_hdr else MUTED
            fg_a    = "white"  if is_hdr else PINK
            f_name  = fh if is_hdr else fn
            f_num   = fh if is_hdr else fm
            f_amt   = fh if is_hdr else fa
            f_due   = fh if is_hdr else fm

            P = 0   # uniform cell padding (px top/bottom)

            # ── right side (packed first so they pin to the right edge) ──
            if is_hdr:
                ...
                ctk.CTkButton(r, text="✕  REMOVE", width=90, height=CELL_H,
                              ...
                              text_color=PINK, hover_color="#280010",
                              font=ctk.CTkFont(family=MONO, size=10, weight="bold"),
                              corner_radius=0,
        ...
        # Header
        _build_row(tbl, BLUE, "#", "NAME", "AMOUNT", "DUE DATE", is_hdr=True)

        # Data rows
        for i, c in enumerate(sorted_list):
            bg = STRIP if i % 2 == 0 else PANEL
